...
__global__ void matrixVecMultiply(float *A, float *B, float *C, int numRows,
                                  int numCols) {
  //@@ Insert code to implement matrix multiplication here
  int row = blockIdx.x * blockDim.x + threadIdx.x;
  if (row < numRows) {
    for (int col = 0; col < numCols; ++col) {
      C[row * numCols + col] = A[row * numCols + col] * B[col];
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  dim3 dimGrid(ceil(numCRows / (float)dimBlock.x), 1, 1);

  wbTime_start(Compute, "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  matrixVecMultiply<<<dimGrid, dimBlock>>>(deviceA, deviceB, deviceC, numCRows,
                                           numCColumns);
