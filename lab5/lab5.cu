...
__global__ void total(float *input, float *output, int len) {
  ...
  __shared__ float shared_sum[2 * BLOCK_SIZE];
  int tx = threadIdx.x;
  int bx = 2 * blockDim.x * blockIdx.x;

  if (tx + bx < len) {
    shared_sum[tx] = input[tx + bx];
  } else {
    shared_sum[tx] = 0.0f;
  }

  if (tx + bx + BLOCK_SIZE < len){
    shared_sum[BLOCK_SIZE+tx] = input[tx + bx + BLOCK_SIZE];
  }
  else{
    shared_sum[BLOCK_SIZE+tx] = 0.0f;
  }
